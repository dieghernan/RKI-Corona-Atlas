...
return results}
function on(obj,eventName,func){if(obj.addEventListener){obj.addEventListener(eventName,func,!1)}else if(obj.attachEvent){obj.attachEvent(eventName,func)}}
function off(obj,eventName,func){if(obj.removeEventListener)
obj.removeEventListener(eventName,func,!1);else if(obj.detachEvent){obj.detachEvent(eventName,func)}}
...
if(!elementData(el,"initialized")){initSizing(el);elementData(el,"initialized",!0);if(bindingDef.initialize){var result=bindingDef.initialize(el,el.offsetWidth,el.offsetHeight);elementData(el,"init_result",result)}}
...
function has_jQuery3(){if(!window.jQuery){return!1}
...
results.push(item)}
return results};window.HTMLWidgets.transposeArray2D=function(array){if(array.length===0)return array;var newArray=array[0].map(function(col,i){return array.map(function(row){return row[i]})});return newArray};function splitWithEscape(value,splitChar,escapeChar){var results=[];var escapeMode=!1;var currentResult="";for(var pos=0;pos<value.length;pos++){if(!escapeMode){if(value[pos]===splitChar){results.push(currentResult);currentResult=""}else if(value[pos]===escapeChar){escapeMode=!0}else{currentResult+=value[pos]}}else{currentResult+=value[pos];escapeMode=!1}}
