...
# file.remove("./assets/geo/country_shapes.geojson")
 ...
 rki_a <- rki_data %>% select(ISO3_CODE) %>% unique()

 rki_err <- rki_data %>% filter(!ISO3_CODE %in% geo_shapes$ISO3_CODE )

 geo_shapes_not_include <- geo_shapes %>% st_drop_geometry() %>% filter(!ISO3_CODE %in% rki_data$ISO3_CODE)
