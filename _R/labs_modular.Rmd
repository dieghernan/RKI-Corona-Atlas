...
data <- readr::read_csv("../assets/data/db_scraped.csv") %>%
        mutate(NAME = switch(params$lang, "en"=NAME_ENGL, "de"=NAME_DE)) %>%
        mutate(risk_level = switch(params$lang, "en"=risk_level_en, "de"=risk_level_de))
...
data_cntries <-
  ...
  filter(is.na(region)) %>%
  left_join(rl) %>%
  select(ISO3_CODE, NAME, REG_EXCLUDED, risk_date, risk_level_code, risk_level)
