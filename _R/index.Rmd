...
<p class="text-right font-weight-bold">Updated in Deutsch: `r format(date, format="%d.%m.%Y")`</p>


## Mappend

Some texten in Deutsch explaining whateveren...
...
labelsDEU <- sprintf("<strong>%s</strong><br/>%s",
                     DEU$label_de,
                     DEU$risk_level_de) %>% lapply(htmltools::HTML)

label0 <- sprintf("<strong>%s</strong><br/>%s",
                  level0$label_de,
                  level0$risk_level_de) %>% lapply(htmltools::HTML)

label1 <- sprintf("<strong>%s</strong><br/>%s",
                  level1$label_de,
                  level1$risk_level_de) %>% lapply(htmltools::HTML)


label2 <- sprintf("<strong>%s</strong><br/>%s",
                  level2$label_de,
                  level2$risk_level_de) %>% lapply(htmltools::HTML)

label3 <- sprintf("<strong>%s</strong><br/>%s",
                  level3$label_de,
                  level3$risk_level_de) %>% lapply(htmltools::HTML)

label4 <- sprintf("<strong>%s</strong><br/>%s",
                  level4$label_de,
                  level4$risk_level_de) %>% lapply(htmltools::HTML)
...
map <- leaflet(options = leafletOptions(minZoom = 0.5)) %>%
  ...
  htmlwidgets::onRender(
    "
        function() {
            $('.leaflet-control-layers-overlays').prepend('<strong>Risk level in Deutsch</strong>');
        }
    "
  )
map
...
Some text in Deutsch explaining whatever...
...
df <- st_drop_geometry(all_shapes) %>%
  select(label_de, risk_level_de)

names(df) <- c("Country/Region in Deutsch", "Risk Level in Deutsch") #RHH
