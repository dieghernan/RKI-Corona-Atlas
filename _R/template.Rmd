...
data_cntries$html_labels <- ifelse(
  ...
  paste0(data_cntries$html_labels,
         "<br />",
         data_cntries$risk_level),
...
data_cntries$html_labelsraw <- data_cntries$html_labels
...
st_write(DEU, paste0("../assets/dist/DEU_", params$lang, ".geojson"))

st_write(level1, paste0("../assets/dist/level1_", params$lang, ".geojson"))

st_write(level2, paste0("../assets/dist/level2_", params$lang, ".geojson"))

st_write(level3, paste0("../assets/dist/level3_", params$lang, ".geojson"))

st_write(level4, paste0("../assets/dist/level4_", params$lang, ".geojson"))
...
df <- st_drop_geometry(all_shapes)  %>%
  select(NAME, risk_level, more_info) %>% as.data.frame()

df[, 3] <- gsub("\r?\n|\r", " ", df[, 3])

# Strip HTML on Details
df$more_info <- gsub("</li><li>", "; ", df$more_info)
df$more_info <- gsub("<.*?>", "", df$more_info)


names(df) <- as.character(idioms[ ,c("country","risk","details")])
